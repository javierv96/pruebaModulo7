...
app.get('/usuarios', async (req, res) => {
    try {
        ...
        //Muestra todos los usuarios con el campo activo TRUE (al crear el usuario por defecto es TRUE)
        const query = await querys.usuariosActivos()
...
app.delete('/usuario', async (req, res) => {
    ...
    try {
        ...
        const usuario = await querys.usuarioTransferencia(id);

        //Si usuario existe en tabla transferencias, desactivarlo
        if(usuario.rows.length > 0){
            const query = await querys.desactivarUsuario(id);
            
            res.json({
                message: `Usuario eliminado correctamente`
            });

        //Si usuario no existe en tabla transferencias, eliminarlo de la tabla usuarios.
        }else {
